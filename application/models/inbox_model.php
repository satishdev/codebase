...
FROM ((SELECT ps.id as sch_id,su.id,ps.start_date,ps.end_date, CONCAT(p.first_name,' ',p.last_name) AS pname,
				...
				WHERE su.user_id='".$user_id."' AND su.is_approve='0' AND su.user_type='1' AND ps.schedule_type='2' order by ps.created_added desc)
				 UNION
				...
				LEFT JOIN clubs c ON sd.club_id=c.id
				LEFT JOIN courts crts ON c.id=crts.clubs_id AND sd.court_id=crts.id
				WHERE pt.players_id='".$user_id."' AND su.is_approve='0' AND su.user_type='2' AND ps.schedule_type='3' order by ps.created_added desc)
				UNION

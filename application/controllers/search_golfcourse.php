...
class Search_golfcourse extends MY_Controller {
    ...
    function state_map_change(){
        $country_id=$this->input->post('countries');
        $state_id=$this->input->post('states');
        if($country_id == ''){
            $area_arr=$this->common_model->select_where('*','gama_area',array('s_id'=>$state_id));
        } elseif($state_id ==''){
            $area_arr=$this->common_model->select_where('*','gama_area',array('c_id'=>$country_id));
        } else {
	...
	function search_teetimes()
	{
			...
			$my_search=$this->input->post('my_search');
		   if($my_submit=='TRUE')
		   {
				$country_id=$this->input->post('country_id');
				$state_id=$this->input->post('state_id');
				...
				echo '<script> console.log("ScriptPhp90: '.$fin_date.'") </script>';
			}else if($my_search == 'TRUE'){
				...
				if($this->input->post('my_date')==''){
					$thoiGianHienTai 	=	strtotime("+1 day", time());
					$fin_date			=	date('Y-m-d',$thoiGianHienTai);
					
				}else{
					$tempDate		=	explode("/",$this->input->post('my_date'));
					$fin_date		=	$tempDate[2].'-'.$tempDate[0].'-'.$tempDate[1];
				}
				
				$times			=	$this->session->userdata('times');
				
				$tempPlayer		= 	$this->input->post('my_players');
				$players		=	$tempPlayer=='Select'?'1':$tempPlayer;
				
				echo '<script>
				console.log("ScriptPhp90 country_id: '.$this->input->post('my_country_id').'") ;
				console.log("ScriptPhp90 state_id: '.$this->input->post('my_state_id').'") ;
				console.log("ScriptPhp90 area_id: '.$this->input->post('my_area_id').'") ;
				console.log("ScriptPhp90 course_id: '.$this->input->post('my_course_id').'") ;
				console.log("ScriptPhp90 fin_date: '.$fin_dates.'") ;
				console.log("ScriptPhp90 times: '.$this->session->userdata('times').'") ;
				console.log("ScriptPhp90 players: '.$players.'") ;
				</script>';
				#$country_id=$this->session->userdata('country_id');
				#$state_id=$this->session->userdata('state_id');
				#$area_id=$this->session->userdata('area_id');

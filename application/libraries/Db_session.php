...
// 		 * It can either set to GMT or time(). The pref
	...
	{
		//deleting record from session table starts here ... added by surya
		if(isset($this->userdata['session_id']))
		{
			$this->object->db->query("delete from ". $this->session_table ." where session_id= '". $this->userdata['session_id']."'");
		}
		//deleting record from session table ends here ... added by surya
    ...
    * to use in in views <?=$this->db_session->flashdata('key_string')?>
    */
    function flashdata($key='')
    {
        $flash_key ='';
        if(strlen($key)) $flash_key = $this->flash_key.':old:'.$key;

...
function  country_change(country_id,state_set_unset)
{
	...
	$.ajax({
		...
		$('#area_div').html('<select name="area_id" id="area_id"><option value="">--Any Area--</option></select><input type="hidden" id="area_id_hid" value="">');
...
<!--google map-->
	...
	$lat=$response->CourseInfoResult->Course->lat;
	$lon=$response->CourseInfoResult->Course->lon;
	$nm=$response->CourseInfoResult->Course->nm;
	$promo=$response->CourseInfoResult->Course->shortPromo;
	?>
        ...
        $('.frt')
          ...
          { action: 'addMarkers',
		   ...
		   <?php 
			 ...
			 $promo = preg_replace("/[\r\n]+/","", $promo);
			  ?>
			  {lat:'<?=$lat?>', lng:'<?=$lon?>', data:'<a href="<?=$curPageURL?>"><?=str_replace("'"," ",$nm)?></a><br><?=str_replace("'"," ",$promo)?>'},
...
<option <? if(@$sess_players==4){?> selected="selected"<? }?> value="4">4</option>
	...
	<div id="dialog-modal" title="All Golf Courses Map Addresses">
		...
		<div  class="gmap3 frt" style="z-index:4"></div>
		</div>
		<!--end google map-->
	</div> 
	...
	<? @$fin_date=strtotime($fin_date);?>			  
	  ...
	  <!--</div> end filtration with price-->
          ...
          <div class="arezona_mid">
			...
			<input type="hidden" value="<?=@$sort?>" id="ajax_sort" />
			<input type="hidden" value="<?=@$course_id?>" id="ajax_course_id" />
			<input type="hidden" value="<?=@$players?>" id="ajax_players" />
			<input type="hidden" value="<?=@$fin_date?>" id="ajax_fin_date" />
			<input type="hidden" value="<?=@$times?>" id="ajax_times" />
	  ...
	  ?>
              ...
              <div class="clr"></div>
			    ...
			    <?php /*?><ul>
				  ...
				  <li><a <? if(@$filter=='early'){?> class="bold"<? }?> href="<?=base_url()?>search_golfcourse/sorting/<?=@$sort?>/early/<?=@$course_id?>/<?=@$players?>/<?=@$fin_date?>/<?=@$times?>">Early Brid</a>
                  <? if(@$filter=='early'){?><a href="<?=base_url()?>search_golfcourse/sorting/<?=@$sort?>/all_day/<?=@$course_id?>/<?=@$players?>/<?=@$fin_date?>/<?=@$times?>"><img style="margin-left:176px; height:13px" src="<?=base_url()?>asserts/images/no.png"</a><? }?></li> 
                  <? }?>
				  
				  <? if(@$filter=='10' || @$filter=='all_day'){?>
				  <li><a <? if(@$filter=='10'){?> class="bold"<? }?> href="<?=base_url()?>search_golfcourse/sorting/<?=@$sort?>/10/<?=@$course_id?>/<?=@$players?>/<?=@$fin_date?>/<?=@$times?>">8am - 10am</a>
                  <? if(@$filter=='10'){?><a href="<?=base_url()?>search_golfcourse/sorting/<?=@$sort?>/all_day/<?=@$course_id?>/<?=@$players?>/<?=@$fin_date?>/<?=@$times?>"><img style="margin-left:176px; height:13px" src="<?=base_url()?>asserts/images/no.png"</a><? }?></li> 
                  <? }?>
				  
				  <? if(@$filter=='noon' || @$filter=='all_day'){?>
				  <li><a <? if(@$filter=='noon'){?> class="bold"<? }?> href="<?=base_url()?>search_golfcourse/sorting/<?=@$sort?>/noon/<?=@$course_id?>/<?=@$players?>/<?=@$fin_date?>/<?=@$times?>">10am - Noon</a>
				 <? if(@$filter=='noon'){?><a href="<?=base_url()?>search_golfcourse/sorting/<?=@$sort?>/all_day/<?=@$course_id?>/<?=@$players?>/<?=@$fin_date?>/<?=@$times?>"><img style="margin-left:176px; height:13px" src="<?=base_url()?>asserts/images/no.png"</a><? }?></li> 
                  <? }?>
				 
				  <? if(@$filter=='2' || @$filter=='all_day'){?>
				  <li><a <? if(@$filter=='2'){?> class="bold"<? }?> href="<?=base_url()?>search_golfcourse/sorting/<?=@$sort?>/2/<?=@$course_id?>/<?=@$players?>/<?=@$fin_date?>/<?=@$times?>">Noon - 2pm </a>
				  <? if(@$filter=='2'){?><a href="<?=base_url()?>search_golfcourse/sorting/<?=@$sort?>/all_day/<?=@$course_id?>/<?=@$players?>/<?=@$fin_date?>/<?=@$times?>"><img style="margin-left:176px; height:13px" src="<?=base_url()?>asserts/images/no.png"</a><? }?></li> 
                  <? }?>
				  
				  <? if(@$filter=='afternoon' || @$filter=='all_day'){?>
				  <li > <a <? if(@$filter=='afternoon'){?> class="bold"<? }?> href="<?=base_url()?>search_golfcourse/sorting/<?=@$sort?>/afternoon/<?=@$course_id?>/<?=@$players?>/<?=@$fin_date?>/<?=@$times?>">Afternoon</a>
                <? if(@$filter=='afternoon'){?><a href="<?=base_url()?>search_golfcourse/sorting/<?=@$sort?>/all_day/<?=@$course_id?>/<?=@$players?>/<?=@$fin_date?>/<?=@$times?>"><img style="margin-left:176px; height:13px" src="<?=base_url()?>asserts/images/no.png"</a><? }?></li> 
                  <? }?>
				</ul><?php */?>
              <div class="clr"></div>
            </div>

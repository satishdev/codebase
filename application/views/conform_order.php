...
foreach($result->result() as $row)
{
  ?>  
	...
	<!--<a href="#." onclick="delete_cart(<?php echo $row->gama_add_cart_id?>)">[x] remove</a>-->
			...
			//user billing info
			$row1=$result1->row();
#			echo '<pre>'; print_r($result1->row()); echo '</pre>';
			...
			<table id="cart-contents" class="mod cart" cellpadding="0" cellspacing="0">
		       <tbody>
				...
				<td> 
				<?
				$country_name=$this->common_model->select_where('name','country',array('id'=>$row1->country_id));
				...
				<td>  
				<? 
				$strlen=strlen($credit_card_no);
				$credit_card_no=substr($credit_card_no, -4);
				$strlen=$strlen-4;
				for($i=1;$i<=$strlen;$i++)
				{
				  $html.="*";
				}
				$html=$html.$credit_card_no;
				echo $html;
				?>         
				</td> 
				</tr>
...
<a   href="<?php echo base_url();?>reserve_golfcourse/booking_golfcourse">Conform Your Order</a>
</div>
        </div>
